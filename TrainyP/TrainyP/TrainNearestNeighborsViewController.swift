 ...
 */
class TrainNearestNeighborsViewController {
...
//  }
  ...
  // MARK: - Training
//TODO
    func disableButton(label: String, image: UIImage?) -> Bool{
      return (label=="nothing") || (image == nil) || (label=="Please add a label")//wenn ein label selected wurde
  }
    /*
...
// TODO Screen when succeded or not
...
//  }
  ...
  private func batchProvider() throws -> MLBatchProvider {
      imageLoader = ImageLoader(image: cgImage,
                                    augment: false,
                                    imageConstraint: imageConstraint(model: model))
...
//      let index = imagesByLabel.flatIndex(for: trueLabel, at: indexPath.row)
      let featureValue = try imageLoader.featureValue()
      ...
      if let pixelBuffer = featureValue.imageBufferValue {
        let featureProvider = PersonalizedkNNTrainingInput(image: pixelBuffer, label: trueLabel)
